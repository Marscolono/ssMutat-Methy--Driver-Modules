...
PCC <- function(normal, cancer, network, n.kernal){
    ...
    del <- function(j){
      ...
      #PCCn+1
      deltapcc <- pcc- as.numeric(oldpcc) #deltaPCC=PCCn+1-PCCn
      Deltapcc <- cbind(Deltapcc, deltapcc)
    }
    #matrix M--each column represents the deltapcc of all gene pairs beacuse of a single cancer sample
    
    library(parallel)
    cl <- makeCluster(n.kernal)
    clusterExport(cl, c('normal', 'cancer', "Deltapcc"), envir = .GlobalEnv)
    diff.metha.matrix <- parLapply(cl, 1 : ncol(cancer), del)
    stopCluster(cl)
    ...
    oldpcc <- as.matrix(oldpcc)
    z <- apply(Deltapcc, 1, ssn_score, nn=113, pcc=oldpcc)
    z_net <- function(zt){
      net <- runif(nrow(Deltapcc), min=0,max=0)
      pvalue <- 1 - pnorm(abs(zt))
      net[which(pvalue<0.01)] <- 1
      return(net)
    }
    A <- apply(z, 2, z_net)
    perturb.net <- as.data.frame(A) 
    return(perturb.net)
  }
############################################################################################
...
mutation2matrix <- function(mutation){
  ...
  mutat.matrix <- matrix(0, nrow= length(mutation.genes), ncol= length(table(mute_snv$Sample_ID)))
...
#####################################################################################
# Description, Identify the aberrantly methylated genesfor each sample by the outlier detection method of Hampel filter.
...
ss.drive.net <- function(perturb.matrix, mutat.matrix, DMG.matrix, net){
 ...
 #identify drive nets 
  ...
  drive.nets1 <- drive.nets
  return(mutat.drive.nets) 
  return(methy.drive.nets)
